            ...
            try {
                ...
                if (response.ok) {
                    if (result.autoLogin && result.token) {
                        ...
                        window.location.href = 'wellness-baseline-assessment.html';
                    } else {
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok && result.token) {
                    ...
                    // Check for wellness baseline completion
                    try {
                        ...
                        if (wellnessResponse.ok) {
                            ...
                            console.log('Wellness data from server:', wellnessResult);
                            
                            if (wellnessResult.wellnessData && wellnessResult.wellnessData.scores) {
                                // User has completed baseline - store data and go to dashboard
                                const wellnessData = {
                                    scores: wellnessResult.wellnessData.scores,
                                ...
                                console.log('Existing wellness data loaded from server');
                                window.location.href = 'Dashboard.html';
                            } else {
                                // No baseline data - need assessment
                                localStorage.setItem('needsBaselineAssessment', 'true');
                                console.log('No wellness baseline found, redirecting to assessment');
                                window.location.href = 'wellness-baseline-assessment.html';
                            }
                        } else {
                            // Wellness API failed - check local data
                            console.log('Wellness API failed, checking localStorage');
                            const localWellnessData = localStorage.getItem('wellnessData');
                            if (localWellnessData) {
                                try {
                                    const parsed = JSON.parse(localWellnessData);
                        ...
                        console.log('Wellness check failed, defaulting to assessment flow');
                        localStorage.setItem('needsBaselineAssessment', 'true');
                        window.location.href = 'wellness-baseline-assessment.html';
                    }
                } else {
