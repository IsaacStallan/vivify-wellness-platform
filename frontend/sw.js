// Service Worker for Student performance Hub PWA
const CACHE_NAME = 'performance-hub-v1.0.0';
...
self.addEventListener('push', event => {
  if (event.data) {
    ...
    const options = {
      ...
      tag: data.tag || 'performance-notification',
      data: data.data || {},
      actions: [
        {
          action: 'open',
...
async function syncperformanceData() {
  try {
    ...
    if (offlineData) {
      ...
      // Send data to server
      const response = await fetch('/api/sync-performance-data', {
        method: 'POST',
        headers: {
